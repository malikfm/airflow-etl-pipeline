...
with DAG(
    ...
    dbt_daily_build = BashOperator(
        ...
        bash_command=f"cd {DBT_PROJECT_DIR} && dbt build" + " --vars '{\"execution_date\": \"{{ ds }}\"}'",
    )
    ...
    if dag.params.get("is_full_refresh", False):
        dbt_backfill_build = BashOperator(
            ...
            bash_command=base_backfill_cmd + " --vars '{\"execution_date\": \"{{ ds }}\"}' --full-refresh",
        )
    else:
        dbt_backfill_build = BashOperator(
            task_id="dbt_backfill_build",
            task_display_name="dbt build: backfill",
            bash_command=base_backfill_cmd + " --vars '{\"execution_date\": \"{{ ds }}\"}'",
        )   
